    ...
    { config, ... }:
    {
      ...
      programs.zsh = {
        ...
        shellAliases = {
          ...
          # git — names follow the oh-my-zsh convention (gs is left to
          # ghostscript). Commit aliases trigger the FIDO2 token (PIN + touch),
          # since commit.gpgsign signs with the sk key.
          # status / inspect
          gst = "git status";
          gss = "git status -s";
          gd = "git diff";
