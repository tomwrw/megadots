_: {
  # The Sunshine remote-play host. LAN-scoped ports, its own config left writable so the web UI works.
  den.aspects.sunshine = {
    nixos = _: {
      # Sunshine.s virtual gamepad and keyboard need uinput, and nothing else
      # here does - so it lives with the thing that needs it.
    ...
    # Delivered through provides.to-users because this aspect is host scope and
    # persist.home is read in a user scope.
    provides.to-users.persist.home.directories = [ ".config/sunshine" ];
