...
│   │   └── security/     #     sops, openssh, hardening, fido2
...
├── namespace.nix         # publishes everything above as flake.denful.megadots
...
`core/security/sops.nix` declares `megadots.core.security.sops`, and
`apps/messaging/signal.nix` declares `megadots.apps.messaging.signal`. If you know an
...
[namespace.nix](modules/namespace.nix) publishes everything under `modules/aspects/` as
...
| | |
|---|---|
| `megadots.*` | the **library** - aspects that describe an application or a subsystem and name no host, no user and no machine of mine. `modules/aspects/`. |
...
into the *user* scope where [apps/sync/syncthing.nix](modules/aspects/apps/sync/syncthing.nix)
...
under `modules/aspects/` claims to describe an *application* rather than my fleet, and the
...
[apps/shell/zsh.nix](modules/aspects/apps/shell/zsh.nix) opened with `{ host, ... }:` so two
...
those two. They now come from [core/nix.nix](modules/aspects/core/nix.nix) via
...
4. **`syncthing/<name>/{key,cert,guiPassword}` in `secrets/users/<user>.yaml`** - the
   ...
   ([apps/sync/syncthing.nix](modules/aspects/apps/sync/syncthing.nix)). Miss these and the host
