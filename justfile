...
# Rebuild a remote host. Requires the operator to already be able to
# SSH-auth as {{ user }} via a resident FIDO2 sk credential (token plugged
# in, touched) — authorizedKeys is sk-only.
rebuild HOST:
    NIX_SSHOPTS="-A" nixos-rebuild switch --flake .#{{ HOST }} --target-host {{ user }}@{{ HOST }} --use-remote-sudo {{ cachyos-cache }}
...
rebuild-onhost HOST:
    NIX_SSHOPTS="-A" nixos-rebuild switch --flake .#{{ HOST }} --build-host {{ user }}@{{ HOST }} --target-host {{ user }}@{{ HOST }} --use-remote-sudo {{ cachyos-cache }}
